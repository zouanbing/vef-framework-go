...
- **Login token mechanisms** (`vef.security.token_type`, default `jwt_token`): the two are independent — session control is only available with `opaque_token`.
  - **`jwt_token`** (`AuthTypeJWTToken`): stateless, self-contained access+refresh JWTs; zero server lookup, unrevocable before `exp`. MCP auth is JWT-only by nature.
  - **`opaque_token`** (`AuthTypeOpaqueToken`): a random reference token backed by a server-side `security.SessionStore` (default `MemorySessionStore`; swap `NewRedisSessionStore` via `fx.Decorate` for multi-node). `OpaqueTokenAuthenticator` looks the token up per request (Principal snapshot from the session) and slides its idle TTL; `OpaqueTokenGenerator` opens the session and enforces `vef.security.session.max_concurrent` via `on_exceed` = `reject` or `evict_oldest` (default: new login kicks the earliest device — 挤下线). Sessions are keyed by `HashOpaqueToken` (never the raw token). `AuthResource.Logout` revokes the current session (a no-op for JWT); over-limit login returns `security.ErrTooManyConcurrentSessions` (HTTP 403). The bearer path (`AccessTokenAuthenticator`) dispatches the configured type; `TokenGenerator.Generate(ctx, principal, SessionMeta)` is mechanism-agnostic. The `SessionStore` is exposed in DI (not private) so apps build their own session-admin endpoints (with their own authorization) via `ListByUser` / `Revoke` / `RevokeUser`; a cross-user "all online sessions" view uses the optional `security.SessionInspector` (`ListAll`, type-asserted, mirroring `event.StreamInspector`) implemented by both built-in stores. The Redis store makes every multi-key mutation atomic (MULTI/EXEC) and guards sliding renewal with `SET … XX` so it cannot resurrect a concurrently-revoked session.
