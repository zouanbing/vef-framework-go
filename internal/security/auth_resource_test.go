...
func (s *ChallengeFlowTestSuite) TestResolveChallengeEmptyToken() {
	...
	body := s.ReadResult(resp)
	s.False(body.IsOk())
	s.Equal(result.ErrCodeBadRequest, body.Code)
}
...
func (s *ChallengeFlowTestSuite) TestResolveChallengeInvalidToken() {
	...
	body := s.ReadResult(resp)
	s.False(body.IsOk())
	s.Equal(result.ErrCodeChallengeTokenInvalid, body.Code)
}
...
func (s *ChallengeFlowTestSuite) TestResolveChallengeWrongType() {
	...
	body := s.ReadResult(resp)
	s.False(body.IsOk())
	s.Equal(result.ErrCodeChallengeTypeInvalid, body.Code)
}
...
func (s *ChallengeFlowTestSuite) TestResolveChallengeProviderRejectsResponse() {
	...
	body := s.ReadResult(resp)
	s.False(body.IsOk())
	s.Equal(result.ErrCodeChallengeResolveFailed, body.Code)
}
...
func TestChallengeFlowTestSuite(t *testing.T) {
...
func (s *AuthResourceErrorPathTestSuite) loginRequest() api.Request {
	return api.Request{
		Identifier: api.Identifier{
			Resource: "security/auth",
			Action:   "login",
...
func (s *AuthResourceErrorPathTestSuite) resolveChallengeRequest() api.Request {
	return api.Request{
		Identifier: api.Identifier{
			Resource: "security/auth",
			Action:   "resolve_challenge",
...
func (s *AuthResourceErrorPathTestSuite) TestLoginNonResultError() {
	...
	body := s.ReadResult(resp)
	s.False(body.IsOk())
	s.Equal(result.ErrCodeUnknown, body.Code)

	events := s.publisher.GetPublishedEvents()
	s.Require().Len(events, 1)
	loginEvent := events[0].(*security.LoginEvent)
	s.False(loginEvent.IsOk)
	s.Equal("unexpected db failure", loginEvent.FailReason)
	s.Equal(result.ErrCodeUnknown, loginEvent.ErrorCode)
}
